...
<body>
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                                <div class="breadcome-heading">
                                    <?php if (hasFeature("addEventDawah")) { ?>
                                        <button class="btn btn-success mb-3" id="addNewDawahBtn">New Dawah Event</button>
                                    <?php } ?>
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                    ...
                    <div class="sparkline12-list mg-b-15">
                        <div class="sparkline12-hd">
                            <div class="d-flex justify-content-between align-items-center flex-wrap" style="padding-bottom: 20px;">
                                ...
                                <!-- Left: Heading -->
                                <div>
                                    <h5 class="mb-0">Upcoming Dawah Events</h5>
                                </div>
                                ...
                                <table class="table table-hover table-bordered">
                                    <thead class="thead-dark">
                                        <tr>
                                            ...
                                            <?php if (hasFeature("editEventDawah") || hasFeature("deleteEventDawah")) { ?>
                                                <th style="width:180px;">Actions</th>
                                            <?php } ?>
                                        </tr>
                                    </thead>
                                    <tbody id="eventDawahTableBody">
    ...
    <!-- Add Event Dawah Modal -->
    <div class="modal fade" id="addEventDawahModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">

                <div class="modal-header">
                    <h5 class="modal-title">Create New Event Dawah</h5>
                </div>

                <div class="modal-body">
                    <form id="addEventDawahForm">
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" onclick="addEventDawah()">Add</button>
    ...
    <!-- Edit Event Dawah Modal -->
    <div class="modal fade" id="editEventDawahModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">

                <div class="modal-header">
                    <h5 class="modal-title">Edit Event Dawah</h5>
                </div>

                <div class="modal-body">
                    <form id="editEventDawahForm">
                        <input type="hidden" id="editEventDawahId" name="id">
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" onclick="updateEventDawah()">Update</button>
                    <button class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                </div>
...
const canEditEventDawah   = <?= hasFeature("editEventDawah")   ? "true" : "false" ?>;
const canDeleteEventDawah = <?= hasFeature("deleteEventDawah") ? "true" : "false" ?>;

let addEventDawahModal    = new bootstrap.Modal(document.getElementById('addEventDawahModal'));
let editEventDawahModal   = new bootstrap.Modal(document.getElementById('editEventDawahModal'));
let deleteEventDawahModal = new bootstrap.Modal(document.getElementById('deleteModal'));

let tomSelectAddDawah  = null;
let tomSelectEditDawah = null;
let eventDawahList = [];
let selectedEventDawahId = null;
let sortDirection = {};
...
async function loadUsers() {
    try {
        ...
        if (tomSelectAddDawah)  tomSelectAddDawah.destroy();
        if (tomSelectEditDawah) tomSelectEditDawah.destroy();

        tomSelectAddDawah  = new TomSelect('#add-organisier',  { placeholder: '-- Select Organizer --', allowEmptyOption: true });
        tomSelectEditDawah = new TomSelect('#edit-organisier', { placeholder: '-- Select Organizer --', allowEmptyOption: true });
...
const addNewDawahBtn = document.getElementById('addNewDawahBtn');
if (addNewDawahBtn) {
    addNewDawahBtn.addEventListener('click', () => {
        document.getElementById('addEventDawahForm').reset();
        if (tomSelectAddDawah) tomSelectAddDawah.clear();
        addEventDawahModal.show();
    });
}

loadUsers();

function addEventDawah() {
    const form = document.getElementById('addEventDawahForm');
    const data = new FormData(form);
    data.append("type", "Dawah");

    const addBtn = document.querySelector('#addEventDawahModal .btn-primary');
    ...
    fetch('addEvent.php', { method: 'POST', body: data })
        .then(res => res.json())
        .then(result => {
            addEventDawahModal.hide();
            showSuccess(result.message, loadEventDawah);

            // ✅ Fire off email sending separately — doesn't block the UI
        ...
        .catch(err => {
            console.error("Error adding Event Dawah:", err);
...
function updateEventDawah() {
    const form = document.getElementById('editEventDawahForm');
    fetch('updateEventDars.php', { method: 'POST', body: new FormData(form) })
        .then(res => res.text())
        .then(msg => { editEventDawahModal.hide(); showSuccess(msg, loadEventDawah); })
        .catch(console.error);
}

// ---- Fetch / render ----

async function loadEventDawah() {
    try {
        const res = await fetch("fetchEvents.php?type=Dawah");
        eventDawahList = await res.json();
        renderEventDawah(eventDawahList);
    } catch (err) {
        console.error("Error fetching Event Dawah:", err);
    }
}

function renderEventDawah(list) {
    const tableBody = document.getElementById("eventDawahTableBody");
    tableBody.innerHTML = "";

    if (!list || list.length === 0) {
        const cols = (canEditEventDawah || canDeleteEventDawah) ? 7 : 6;
        tableBody.innerHTML = `<tr><td colspan="${cols}" class="text-center">No Event Dawah found</td></tr>`;
        return;
    }

    list.forEach((event, i) => {
        const actionsCell = (canEditEventDawah || canDeleteEventDawah) ? `
        <td>
            ${canEditEventDawah   ? `<button class="btn btn-sm btn-primary" onclick="editEventDawah(${event.id})">Edit</button>`   : ''}
            ${canDeleteEventDawah ? `<button class="btn btn-sm btn-danger"  onclick="deleteEventDawah(${event.id})">Delete</button>` : ''}
        </td>` : '';
...
function editEventDawah(id) {
    const event = eventDawahList.find(e => e.id == id);
    if (!event) return;

    document.getElementById('editEventDawahId').value = event.id;
    document.getElementById('edit-topic').value        = event.topic;
    if (tomSelectEditDawah) tomSelectEditDawah.setValue(event.organisier); // ✅ ID via Tom Select
    ...
    editEventDawahModal.show();
}

function deleteEventDawah(id) {
    selectedEventDawahId = id;
    const ev = eventDawahList.find(e => e.id == id);
    document.getElementById("deleteMessage").innerText =
        `Are you sure you want to delete "${ev ? ev.topic : 'this event'}"?`;
    deleteEventDawahModal.show();
}

document.getElementById("confirmDeleteBtn").addEventListener("click", function() {
    if (!selectedEventDawahId) return;
    fetch(`deleteEvent.php?id=${selectedEventDawahId}`)
        .then(res => res.text())
        .then(() => {
            loadEventDawah();
            deleteEventDawahModal.hide();
            selectedEventDawahId = null;
        });
});
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    const filtered = eventDawahList.filter(event => {
    ...
    renderEventDawah(filtered);
});

function sortTable(column) {
    sortDirection[column] = !sortDirection[column];

    eventDawahList.sort((a, b) => {
        let valA = a[column] ?? '';
        let valB = b[column] ?? '';
