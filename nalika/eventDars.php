...
<body>
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                                <div class="breadcome-heading">
                                    <?php if (hasFeature("addDars")) { ?>
                                        <button class="btn btn-success mb-3" id="addNewDarsBtn">New Dars Event</button>
                                    <?php } ?>
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                    ...
                    <div class="sparkline12-list mg-b-15">
                        <div class="sparkline12-hd">
                            <div class="d-flex justify-content-between align-items-center flex-wrap" style="padding-bottom: 20px;">
                                ...
                                <!-- Left: Heading -->
                                <div>
                                    <h5 class="mb-0">Upcoming Daroos</h5>
                                </div>
                                ...
                                <table class="table table-hover table-bordered">
                                    <thead class="thead-dark">
                                        <tr>
                                            ...
                                            <?php if (hasFeature("editDars") || hasFeature("deleteDars")) { ?>
                                                <th style="width:180px;">Actions</th>
                                            <?php } ?>
                                        </tr>
                                    </thead>
                                    <tbody id="eventDarsTableBody">
    ...
    <!-- Add Event Dars Modal -->
    <div class="modal fade" id="addEventDarsModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">

                <div class="modal-header">
                    <h5 class="modal-title">Create New Event Dars</h5>
                </div>

                <div class="modal-body">
                    <form id="addEventDarsForm">
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" onclick="addEventDars()">Add</button>
    ...
    <!-- Edit event Dars Modal -->
    <div class="modal fade" id="editEventDarsModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">

                <div class="modal-header">
                    <h5 class="modal-title">Edit Event Dars</h5>
                </div>

                <div class="modal-body">
                    <form id="editEventDarsForm">
                        <input type="hidden" id="editEventDarsId" name="id">
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" onclick="updateEventDars()">Update</button>
                    <button class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                </div>
    ...
    <script>
        ...
        let addEventDarsModal = new bootstrap.Modal(document.getElementById('addEventDarsModal'));

        // Show modal when "New Dars Event" button is clicked
        document.getElementById('addNewDarsBtn').addEventListener('click', () => {
            // Clear form fields
            document.getElementById('addEventDarsForm').reset();
            addEventDarsModal.show();
        });

        // Load users into organizer dropdowns
            ...
            try {
                ...
                ['add-organisier', 'edit-organisier'].forEach(id => {
                    ...
                    users.forEach(u => {
                        ...
                        opt.value = u.id; // store ID as value since DB stores ID
        ...
        const addNewDarsBtn = document.getElementById('addNewDarsBtn');
        if (addNewDarsBtn) {
            addNewDarsBtn.addEventListener('click', () => {
                document.getElementById('addEventDarsForm').reset();
                addEventDarsModal.show();
            });
        }
        ...
        // Add Event Dars
function addEventDars() {
    const form = document.getElementById('addEventDarsForm');
    const data = new FormData(form);
    data.append("type", "Dars");

    const addBtn = document.querySelector('#addEventDarsModal .btn-primary');
    ...
    fetch('addEvent.php', { method: 'POST', body: data })
        ...
        .then(result => {
            ...
            // ✅ Fire off email sending separately — doesn't block the UI at all
        ...
        .catch(err => {
            console.error("Error adding Event Dars:", err);
    ...
    <script>
        let eventDarsList = []; // global array

        // Load Event Dars from server
        async function loadEventDars() {
            try {
                const res = await fetch("fetchEvents.php?type=Dars");
                eventDarsList = await res.json();
                renderEventDars(eventDarsList);
            } catch (err) {
                console.error("Error fetching Event Dars:", err);
            }
        }

        // Render event dars table
function renderEventDars(list) {
    const tableBody = document.getElementById("eventDarsTableBody");
    tableBody.innerHTML = "";

    if (!list || list.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="7" class="text-center">No Event Dars found</td></tr>`;
        return;
    }
    ...
    list.forEach((event, i) => {
        tableBody.innerHTML += `
            <tr>
                <td>${i + 1}</td>
                <td>${event.topic}</td>
                <td>${event.organisierName ?? '-'}</td>   <!-- changed -->
                <td>${formatDateTime(event.dateTime)}</td>
                <td>${event.phone ?? '-'}</td>
                <td>${event.location ?? '-'}</td>
                <td>
                <?php if (hasFeature("editDars")) { ?>
                    <button class="btn btn-sm btn-primary" onclick="editEventDars(${event.id})">Edit</button>
                <?php } ?>
                <?php if (hasFeature("deleteDars")) { ?>
                    <button class="btn btn-sm btn-danger" onclick="deleteEventDars(${event.id})">Delete</button>
                <?php } ?>
                </td>
            </tr>
        `;
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    const filtered = eventDarsList.filter(event => {
    ...
    renderEventDars(filtered);
        ...
        // Edit Event Dars modal
        let editEventDarsModal = new bootstrap.Modal(document.getElementById('editEventDarsModal'));

        function editEventDars(id) {
            const event = eventDarsList.find(e => e.id == id);
            if (!event) return;

            document.getElementById('editEventDarsId').value = event.id;
            document.getElementById('edit-topic').value = event.topic;
            
                const editSel = document.getElementById('edit-organisier');
    if (editSel.tomselect) {
        editSel.tomselect.setValue(event.organisier); // ID value
    }
            ...
            editEventDarsModal.show();
        }

        // Update Event Dars
        function updateEventDars() {
            const form = document.getElementById('editEventDarsForm');
            const data = new FormData(form);

            fetch('updateEventDars.php', {
                    method: 'POST',
                    body: data
                })
                .then(res => res.text())
                .then(msg => {
                    editEventDarsModal.hide();
        ...
        // Delete Event Dars
        let selectedEventDarsId = null;
        let deleteEventDarsModal = new bootstrap.Modal(document.getElementById('deleteModal'));

        function deleteEventDars(id) {
            selectedEventDarsId = id;
            const row = event.target.closest("tr");
            const topic = row.children[1].innerText;
            document.getElementById("deleteMessage").innerText =
                `Are you sure you want to delete "${topic}"?`;
            deleteEventDarsModal.show();
        }

        document.getElementById("confirmDeleteBtn").addEventListener("click", function() {
            if (!selectedEventDarsId) return;
            fetch(`deleteEvent.php?id=${selectedEventDarsId}`)
                .then(res => res.text())
                .then(() => {
                    loadEventDars();
                    deleteEventDarsModal.hide();
                    selectedEventDarsId = null;
                });
        });
