...
function initSidebarNav() {
  ...
  menu.querySelectorAll('.has-arrow').forEach(function (link) {
    link.addEventListener('click', function (e) {
      ...
      var isCurrentlyOpen = parent.classList.contains('active') || window.getComputedStyle(submenu).display !== 'none';
...
var sparkConfigs = [
    ...
    // ── These 3 now use real DB data ──────────────────────────────────────
    { id: 'sparkline8',  data: (typeof quranTrendData  !== 'undefined') ? quranTrendData  : [0,0,0,0,0,0,0,0], color: '#00c292' },
    { id: 'sparkline9',  data: (typeof darsTrendData   !== 'undefined') ? darsTrendData   : [0,0,0,0,0,0,0,0], color: '#13dafe' },
    { id: 'sparkline10', data: (typeof combinedTrend   !== 'undefined') ? combinedTrend   : [0,0,0,0,0,0,0,0], color: '#00c292' },
...
function initProjectDashboard() {
  ...
  // ── Read PHP-injected values safely ──────────────────────────────────────
  var areaCount      = (typeof pmAreaCount  !== 'undefined') ? pmAreaCount  : 0;
  var totalUsers     = (typeof pmTotalUsers !== 'undefined') ? pmTotalUsers : 0;
  ...
  if (taskEl) {
    new ApexCharts(taskEl, {
      ...
      series: [
        { name: 'Dars Sessions',  data: [8, 10, 9, 12, 10, 11] },
        { name: 'Workshops',      data: [2,  1, 3,  2,  3,  2] },
        { name: 'Events',         data: [1,  2, 1,  3,  2,  1] },
        { name: 'New Members',    data: [3,  5, 4,  6,  4,  5] },
      ],
      colors: ['#24caa1', '#03a9f4', '#ab8ce4', '#f8ac59'],
      plotOptions: { bar: { columnWidth: '50%', borderRadius: 3 } },
  ...
  // ── Sprint Burndown (kept as-is — replace with real data later) ───────────
